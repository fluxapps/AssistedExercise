...
class ilAnswerListInputGUI extends ilFormPropertyGUI {
	...
	function hasUserVotedForOneAnswer() {
		$voting = xaseVoting::where(array('user_id' => $this->dic->user()->getId(), 'question_id' => $this->xase_question->getId()))->first();
		if (empty($voting)) {
			return false;
		}
		return true;
	...
	public function insert($a_tpl) {
		...
		$tpl->setVariable("ITEM", $this->xase_question->getTask());
		$item_creator = xaseilUser::where(array('usr_id' => $this->xase_question->getUserId()))->first();
		...
		foreach ($this->getAnswers() as $answer) {
			if ($answer->getAnswerStatus() == xaseAnswer::ANSWER_STATUS_CAN_BE_VOTED) {
				...
				$tpl->setVariable("VOTE_ERROR_TEXT", $this->obj_facade->getLanguageValue("vote_error_text"));
				...
				$answer_creator = xaseilUser::where(array('usr_id' => $answer->getUserId()))->first();
...
//				if($this->getModus()!= 3) {
				...
				if (count($this->comments) >= 2) {
					$tpl->setVariable("COMMENT_TEXT", $this->obj_facade->getLanguageValue('comments'));
				} else {
					$tpl->setVariable("COMMENT_TEXT", $this->obj_facade->getLanguageValue('comment'));
				}
					...
					if (empty($this->comments)) {
						...
						foreach ($this->comments as $comment) {
							...
							$tpl_comment_form->setVariable("COMMENT", $this->comment_non_editable_value_gui->render());
							$comment_creator = xaseilUser::where(array('usr_id' => $comment->getUserId()))->first();
							$tpl_comment_form->setVariable("COMMENT_CREATOR", $comment_creator->getFirstname() . " " . $comment_creator->getLastname());
							$tpl_comment_form->parseCurrentBlock();
						}
					}
					$tpl_comment_form->setCurrentBlock("comment_wrapper");
					$tpl_comment_form->setVariable("CREATE_COMMENT_LINK_TEXT", $this->obj_facade->getLanguageValue('add_comment'));
					$tpl_comment_form->setVariable("CREATE_COMMENT_FORM_LABEL", $this->obj_facade->getLanguageValue('add_new_comment'));
					$tpl_comment_form->setVariable("CREATE_COMMENT_FORM_ERROR_MESSAGE", $this->obj_facade->getLanguageValue('create_comment_form_error_message'));
					$tpl_comment_form->setVariable("COMMENT_SAVE_TEXT", $this->obj_facade->getLanguageValue('save'));
					$tpl_comment_form->setVariable("COMMENT_DISCARD_TEXT", $this->obj_facade->getLanguageValue('discard_comment'));
					$tpl_comment_form->setVariable("ANSWER_ID", $answer->getId());
					$tpl_comment_form->parseCurrentBlock();
...
//				}
		...
		if (!empty($_POST)) {
			...
			foreach ($_POST['answer'] as $id => $data) {
				if (!$this->checkInput()) {
					ilUtil::sendFailure($this->obj_facade->getLanguageValue("msg_vote_for_at_least_one_answer"));
				}
...
//						}
	...
	public function getXaseItem() {
		return $this->xase_question;
	}


	/**
	 * @param mixed $xase_question
	 */
	public function setXaseItem($xase_question) {
